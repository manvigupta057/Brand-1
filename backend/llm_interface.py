...
def generate_answer(query: str, context_chunks: list[str], chat_history: list[dict], system_instruction: str = None) -> dict:
    ...
    with mlflow.start_run(run_name=f"Chat: {query[:20]}"):
        ...
        if is_greeting:
            # 🚀 Super Force Fix: Simple hello back
            messages = [{"role": "system", "content": f"{base_guideline}. The user is just greeting you. Just say hello back in your persona. DO NOT provide any data or metrics."}]
            messages.append({"role": "user", "content": query})
            prompt = 'Return ONLY a JSON object with keys "answer" and "brand_type" (set brand_type to null).'
        else:
            ...
            # Strategic analysis prompt only for real questions
            prompt = f"""
            [[ BRAND CONTEXT ]]
            {context}
